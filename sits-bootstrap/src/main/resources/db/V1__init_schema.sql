...
CREATE TABLE inventory_risk (
    ...
    sku_id              BIGINT          NOT NULL                   COMMENT 'SKU ID',
    warehouse_id        BIGINT          NOT NULL                   COMMENT '仓库ID',
    scan_batch_no       VARCHAR(64)     DEFAULT NULL               COMMENT '扫描批次号',
    latest_scan_time    DATETIME        DEFAULT NULL               COMMENT '最新扫描时间',
    risk_type           VARCHAR(32)     NOT NULL                   COMMENT '风险类型: SHORTAGE/OVERSTOCK',
    risk_level          VARCHAR(32)     NOT NULL                   COMMENT '风险等级: HIGH/MEDIUM/LOW',
    risk_fingerprint    VARCHAR(128)    NOT NULL                   COMMENT '风险指纹(skuId_warehouseId_riskType)',
    ...
    UNIQUE KEY uk_sku_wh_risk_fp (sku_id, warehouse_id, risk_type, risk_fingerprint),
    KEY idx_sku_warehouse (sku_id, warehouse_id),
    KEY idx_scan_batch_no (scan_batch_no),
    KEY idx_status (status),
...
CREATE TABLE transfer_order (
    ...
    status                  VARCHAR(32)     NOT NULL DEFAULT 'CREATED' COMMENT '状态: CREATED/APPROVING/APPROVED/REJECTED/OUTBOUNDING/OUTBOUNDED/IN_TRANSIT/INBOUNDING/COMPLETED/CANCELLED/FAILED',
